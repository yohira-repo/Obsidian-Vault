...
- **社内LAN（VLAN 1〜3）はIPv4のみ**。IPv6はHGW–RTX1300間のWAN区間のみで使用
- VLAN1：営業・総務管理（192.168.128.0/24）
- VLAN2：開発メンバ席（192.168.64.0/24）
- VLAN3：ゲスト専用（192.168.192.0/24）
- 10GbE区間は **CAT6A** を前提
...
- **Wi-Fi（無線LAN）** は2.4GHz／5GHzともSSID発信をOFFにします（社内無線はBE9400に集約）。
- **ルータの接続先設定は投入しません**（IPv4 over IPv6はRTX1300で終端）。
- **LAN1〜3は未使用**とし、LAN4のみを使用します。
...
| 社内VLAN 1〜3のIPv6 | **配布しない（IPv4のみ）** | RA方式ではプレフィックスを切り出せないため |
| 社内VLAN 1〜3のIPv4 | RTX1300がDHCP配布・NAPTでインターネットへ | 従来のPPPoE運用と同等 |
...
> **VLAN3はゲスト専用**です。社内の無線端末はOffice-WiFi（VLAN1）／Develop-WiFi（VLAN2）に収容し、有線と同じセグメントに入ります。
...
|---:|---|---|---|---|---|---|
| VLAN 1 | 営業・総務管理 | `192.168.128.0/24` | `192.168.128.1` | `.100`〜`.149`（予約）／`.150`〜`.199`（自動） | `.2` 〜 `.99` | `.200` 〜 `.254` |
| VLAN 2 | 社内開発・試験 | `192.168.64.0/24` | `192.168.64.1` | `.100`〜`.149`（予約）／`.150`〜`.199`（自動） | `.2` 〜 `.99` | `.200` 〜 `.254` |
| VLAN 3 | ゲスト専用 | `192.168.192.0/24` | `192.168.192.1` | `.100` 〜 `.199`（自動のみ） | 割当なし | `.200` 〜 `.254` |
...
| 案B：VLAN1に参加させる | LAN1をブリッジ等でVLAN1（`192.168.128.0/24`）に収容する | 保守端末から業務セグメントへ直接アクセスできるが、設定が複雑になり分離のメリットを失う |

> 本設計では **案A（`192.168.200.0/24`）を採用**します。保守ターミナルからVLAN 1・2へアクセスする必要がある場合は、RTX1300のフィルタで必要な通信のみ許可します。

### 3-3. 固定IP割当（VLAN 1 / `192.168.128.0/24`）

| 機器 | 接続先 | IPアドレス | 備考 |
|---|---|---|---|
| RTX1300（VLAN1 GW） | － | `192.168.128.1` | 確定 |
| XS508TM（管理IP） | Port 1 経由 | `192.168.128.2`（案） | マネージドSWのため管理IPが必要 |
| WBE710（BE9400）管理IP | PoEエクステンダー経由 | `192.168.128.3`（案） | AP管理用（NETGEAR Insight利用時も要） |
| DocuCentre-VII C2273（複合機） | XS508TM Port 6 直結 | `192.168.128.20` | 確定。**VLAN1専用**。社内は原則紙印刷禁止のため、スキャン等の利用が主体 |
...
| 通信方向 | 可否 | 実装・備考 |
|---|:--:|---|
| VLAN 1 → VLAN 2 | **許可** | **動的フィルタ（`ip filter dynamic`）による片方向許可**。VLAN1発のセッションと、その戻り通信のみ通す |
| VLAN 2 → VLAN 1 | **拒否** | **例外なし**（VLAN2は紙媒体を扱わないため印刷・スキャンの例外も不要） |
| 複合機（`192.168.128.20`） → VLAN 2 | **拒否** | VLAN 1 → VLAN 2 許可の**例外**。VLAN2は紙媒体を直接・間接に利用しないため、複合機発のVLAN2向け通信を静的フィルタで遮断し、複合機の通信範囲をVLAN 1 内に限定する |
| VLAN 3（ゲスト） → VLAN 1・2 | **拒否** | ゲスト分離 |
| VLAN 3（ゲスト） → インターネット | 許可 | ゲストはインターネット接続のみ |
| VLAN 1 → AWS VPC（`10.0.3.0/24`） | **許可** | 拠点間IPsec経由。業務系のみ |
| VLAN 2・VLAN 3 → AWS VPC | **拒否** | 開発系・ゲストからAWSへは接続しない |
| VPN業務ユーザー（`192.168.201.16/28`） → VLAN 1 | **許可** | **TCP 3389（RDP）／TCP 22（SSH）のみ**。他ポートは拒否 |
| VPN開発ユーザー（`192.168.201.32/28`） → VLAN 2 | **許可** | **TCP 3389（RDP）／TCP 22（SSH）のみ**。他ポートは拒否 |
| VPNユーザー → 上記以外（他VLAN・複合機・AWS・保守セグメント） | **拒否** | 最小権限。用途はリモートデスクトップ／SSHに限定 |
| 保守セグメント → VLAN 1・2 | 許可 | 保守作業用 |
| VLAN 1・2・3 → 保守セグメント | 拒否 | 保守は片方向（保守 → 業務）のみ |
| VLAN 1・2 → インターネット | 許可 | 固定グローバルIPv4によるNAPT |
| 各VLAN → HGW（`192.168.1.0/24`） | 拒否 | 保守時の一時直結でのみアクセス |

#### 実装上の注意

- **「片方向許可」はセッション開始方向の話**です。静的フィルタ（`ip filter`）だけで「VLAN1→VLAN2 を許可、VLAN2→VLAN1 を拒否」と設定すると**戻りパケットも落ちて通信不能**になります。必ず動的フィルタ（`ip filter dynamic`）でステートフルに扱ってください。
- **VLAN 2 は紙媒体を直接・間接に利用しません。** したがって複合機（`192.168.128.20`）とVLAN 2 の間の通信は、印刷・スキャンとも**双方向で一切不要**です。
  - VLAN2 → 複合機（印刷、管理画面、スキャンボックス取得）：VLAN2→VLAN1の拒否により遮断されます。**例外許可は設けません。**
  - 複合機 → VLAN2（スキャンのSMB／FTP送信）：VLAN1→VLAN2は原則許可のため通ってしまいます。これを防ぐため、**送信元 `192.168.128.20` からVLAN 2 宛の通信を静的フィルタで明示的に拒否**し、動的フィルタの許可より前段に配置します。
  - 結果として複合機の通信範囲はVLAN 1 内（＋必要な外部通信）に限定されます。複合機はファームウェア更新頻度が低く踏み台になりやすい機器のため、この制限は防御上も有効です。
- AKERUN アクセスポイントはVLAN1に所属し、クラウドへの外向き通信（インターネット方向）を許可します。
- AWS向けの通信はVLAN1のみのため、**開発・検証用途でAWSへ接続する必要が生じた場合は本方針の見直し**が必要です。
...
| `192.168.64.0/24` | VLAN 2 | ✅ 重複なし |
| `192.168.128.0/24` | VLAN 1 | ✅ 重複なし |
| `192.168.192.0/24` | VLAN 3 | ✅ 重複なし |
...
|---|---|---|---|
| 業務ユーザー | `192.168.201.16/28`（`.17`〜`.30`） | VLAN 1 `192.168.128.0/24` のみ | TCP 3389（RDP）／TCP 22（SSH） |
| 開発ユーザー | `192.168.201.32/28`（`.33`〜`.46`） | VLAN 2 `192.168.64.0/24` のみ | TCP 3389（RDP）／TCP 22（SSH） |

- 業務ユーザーは AWS VPC・VLAN 2・VLAN 3・複合機・保守セグメントへ到達できません。
- 開発ユーザーは VLAN 1（複合機含む）・AWS VPC・VLAN 3・保守セグメントへ到達できません。
...
- **MACアドレス台帳の作成が必要**です（PC名・利用者・MACアドレス・予約IPの対応表）。
