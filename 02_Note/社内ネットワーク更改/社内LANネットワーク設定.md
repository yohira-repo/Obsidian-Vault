...
- 回線は **フレッツ光クロス（10G）** で、**10G-EPON ONU（光加入者終端装置・別体）→ NTT XG-200KI（HGW）** を経由（XG-200KIはONU一体型ではない）
- XG-200KIは **Wi-Fi OFF・LAN1〜3未使用・ルータ接続先設定なし**（パススルー運用）で、**LAN4 → RTX1300 LAN2** に接続
...
| 区間                          | 接続内容      | 帯域 / 備考                      |
| --------------------------- | --------- | ---------------------------- |
| フレッツ光クロス → 10G-EPON ONU  | 回線引き込み（光終端）    | 10Gbps / 光ファイバ・NTT工事           |
| 10G-EPON ONU → XG-200KI WAN | 光→電気の受け渡し | 10GBASE-T / CAT6A（**ONUは別体**）      |
| XG-200KI LAN4 → RTX1300 LAN2      | WAN接続      | 10GbE / CAT6A、HGWはパススルー運用      |
...
| ② | LAN2 RJ-45 | XG-200KI（HGW） LAN4 | 10GbE対応コンボポート（WAN） |
...
- インターネット回線は、**XG-200KI（HGW）のLAN4** から **LAN2のRJ-45ポート**へ接続します。回線とRTX1300は直結ではありません。
- XS508TMへの10GbE幹線は **LAN3のRJ-45ポート**です。
- LAN2・LAN3のSFP+スロットは、対応するRJ-45ポートと排他のため、この構成では使用しません。

### XG-200KI（HGW）ポート使用図

フレッツ光クロスでは、**別体の 10G-EPON ONU（光加入者終端装置・箱型）**で光を終端し、その配下にXG-200KIが必須で介在します（XG-200KIはONU一体型ではありません）。XG-200KIのWANは光ではなく **10GBASE-T（RJ-45）**で、ONUの10GBASE-T端子とCAT6Aで接続します。ルータ機能を持つ機器ですが、本構成では接続先設定を投入せず、RTX1300が固定グローバルIPを直接保持します。

#### ポート割当

| 番号 | ポート | 接続先 | 用途 / 備考 |
|---:|---|---|---|
| ① | WAN（10GBASE-T / RJ-45） | **10G-EPON ONU** の10GBASE-T端子 | 10Gbps / CAT6A（**ONUは別体**） |
| ② | LAN4（10GBASE-T） | RTX1300 LAN2 RJ-45 | 10GbE幹線、CAT6A |
| － | LAN1〜3（1000BASE-T） | 未使用 | 保守時のみ管理端末を一時接続 |

> 前段の 10G-EPON ONU（光ファイバ側で終端）は、`光ファイバ → ONU → XG-200KI WAN` の順で接続します。
...
- **Wi-Fi（無線LAN）は常時OFF**（XG-200KIはWi-Fi 7対応のため 2.4GHz／5GHz／**6GHz** の3帯域ともSSID発信を停止）。社内無線はBE9400に集約するため、平常時・メンテナンス時とも使用しません。
- **ルータの接続先設定は投入しません**（IPv4 over IPv6はRTX1300で終端）。
- **平常時に使用するのはLAN4のみ**。LAN1〜3は平常時未使用とし、**メンテナンス時（HGW管理画面へのアクセス時）にのみ使用**します。
- XG-200KIがNTT網からDHCPv6-PDでプレフィックスを取得し、**LAN側へはRA方式で /64 を1本広告**します。配下ルータへのプレフィックス再委任は行われません。
- XG-200KIには従来機（PR-500KI等）の完全なブリッジモードがないため、**LAN側 192.168.1.1/24 とDHCPサーバは動作したまま**になります。社内VLANのサブネットと重複させないでください。

#### HGW管理画面へのアクセス運用

- 通常運用では、HGW管理画面（`http://192.168.1.1/`）への常設経路は設けません。
- 設定変更・障害切り分けが必要な場合のみ、**保守ターミナルをXG-200KIのLAN1〜3いずれかへ一時的に直結**してアクセスし、作業後はケーブルを撤去します。
- 一時直結時は保守ターミナル側を DHCP（192.168.1.0/24）で受け取る想定です。

### IPv6アドレス配布方式とIPv4/IPv6の扱い

光クロスのIPv6アドレス配布方式は原則DHCPv6-PDですが、**XG-200KI配下にIPoEルータを設置する場合は例外的にRA方式**となります。RTX1300はRA方式で設定します。

```text
NTT網 ──(DHCPv6-PD /60)──▶ XG-200KI ──(RA で /64 を1本広告)──▶ RTX1300
                            ▲                                   ▲
...
| （参考） | HGW管理（XG-200KI LAN側） | `192.168.1.0/24` | `192.168.1.1`（HGW） | HGWのDHCP | － | 変更不可 |
...
| **VLAN 20 ↔ AWS VPC（`10.0.3.0/24`）** | **許可（事後対応）** | 【2026-09-14変更】AWS拠点間VPN(VGW)は**開発(VLAN20)専用**（従来VLAN10→VLAN20へ変更）。双方向 |
| VLAN 10・VLAN 30 → AWS VPC | **拒否** | 【2026-09-14変更】AWSはVLAN20のみ。VLAN10/ゲストからは接続しない |
| リモートVPN業務（`192.168.201.16/28`） → VLAN 10 | **許可** | **TCP 3389／22のみ**。業務アカウント（yohira/t.kido/mogik/iizukak）＝VLAN10到達 |
| リモートVPN開発（`192.168.201.32/28`） → VLAN 20 | **許可** | **TCP 3389／22のみ**。【2026-09-15復活】開発アカウント（yohiradev/t.kidodev）＝VLAN20専用到達（将来性のため） |
| リモートVPNユーザー → 上記以外（VLAN30・複合機・保守・AWS、業務→VLAN20、開発→VLAN10） | **拒否** | 到達は払出レンジ＋フィルタで最小化 |
...
- 【2026-09-14変更】**AWS向けの通信はVLAN20（開発）のみ**（従来VLAN10から変更）。VLAN10/30からAWSへは接続しない。

### 3-5. サブネット重複チェック

| セグメント | 用途 | 重複判定 |
|---|---|:--:|
| `192.168.1.0/24` | XG-200KI（HGW）LAN側 | ✅ 重複なし |
...
> [!info] 【到達先の変遷：2026-09-14→2026-09-15】外部リモートVPNは **業務=VLAN10／開発=VLAN20** の2系統
> 一度（2026-09-14）「VLAN10のみ・開発到達廃止」に縮小したが、**2026-09-15 に将来性を考慮して開発ユーザー→VLAN20専用到達を復活**（ユーザー方針変更）。
> - 業務（`192.168.201.16/28`：yohira/t.kido/mogik/iizukak）→ **VLAN10**（RDP/SSH）
> - 開発（`192.168.201.32/28`：yohiradev/t.kidodev）→ **VLAN20専用**（RDP/SSH）
> - 到達先はトンネルでなく払出IP＋フィルタで決まる（解釈A）。開発アカウントは当面暫定・運用で増減。開発のAWS利用は引き続き拠点間VGW（VLAN20専用）でも担保。
> **詳細な払出レンジ／アカウント／フィルタ番号の確定は VPN実装時（優先度低）** に本方針で更新する。
...
- 保守ターミナルは **RTX1300 の LAN1** へ接続し、設定変更・障害対応用に使用します。HGW管理画面へアクセスする場合のみ、XG-200KIへ一時直結します。
- 回線とRTX1300の間には、**10G-EPON ONU（別体）と XG-200KI（HGW）の2台**が必ず介在します（`光 → ONU → XG-200KI → RTX1300`）。XG-200KIはONU一体型ではありません。撤去・バイパスはできません。
- **ONU と XG-200KI の2台分**の電源口とラックスペースを占有し、**いずれもUPSの保護対象**に含めます（ONU停電＝インターネット全断のため）。**UPSは8個口で口数の不足なし（確認済）**、ONU↔XG間の**CAT6AはONUに付属**。ONUは DC12V/0.7A（≈8.4W）。ONU諸元（部品名・製造番号・MAC）は [[config_XG-200KI]] を参照。
- 無線LANはSSIDごとにVLAN 10・20・30へ分離し、ゲスト用VLAN 30から社内VLAN 10・20への通信はRTX1300で拒否します。
- 本物の画像ベースにしたい場合は、**XG-200KI / XS508TM / LXW-10G8 / RTX1300 / BE9400 / PoEエクステンダー** の前面画像をご提供いただければ、その画像上に「Port 1 = uplink」などの注記を入れる形で、より実機に近い資料へできます。

### HGW関連の確定事項

| 項目 | 決定内容 |
|---|---|
| HGWの介在 | 固定IPサービスでは、ひかり電話契約の有無に関わらず `ONU – XG-200KI – ルータ` が必須構成。**HGWを外した回線直結は不可** |
...
| 項目 | 内容 | 確認先 |
|---|---|---|
| HGW側の必要設定 | 固定IP1利用時にXG-200KI側へ必要な設定（接続先設定の扱い、DMZ／ポート設定の要否） | OCN / NTT東日本 |
| VPNの通過性 | MAP-E上でAWS VGW向けのIKE（UDP 500／4500）およびESPが通過するか | OCN / 社内検証 |
| IPアドレス重複 | XG-200KIのLAN側 192.168.1.0/24 と社内VLANサブネットが重複しないこと（**VLAN毎のIP設計表が未作成**） | 社内 |
