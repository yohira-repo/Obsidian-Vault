...
| リモートVPNユーザー（`192.168.201.0/24`） → VLAN 10 | **許可** | **TCP 3389（RDP）／TCP 22（SSH）のみ**。【2026-09-14変更】**外部VPNの到達はVLAN10のみ** |
| リモートVPNユーザー → VLAN 20/30・複合機・AWS・保守 | **拒否** | 【2026-09-14変更】外部VPNはVLAN10限定（開発のVLAN20リモートは廃止＝開発のAWSは拠点間VGWで担保） |
...
> [!warning] 【2026-09-14 方針変更】外部からのリモートVPNは **VLAN10（営業・総務）のみ**
> 外部からのリモートアクセスVPNの到達先は **VLAN10 に限定**（従来の「開発ユーザー→VLAN20」リモート経路は廃止。開発のAWS利用は **拠点間VPN(VGW)＝VLAN20専用** で担保）。
> このため、以下の本文にある **「開発ユーザー(`192.168.201.32/28`)→VLAN20」** の払出・アカウント・フィルタ（`ip filter 202/203` 等、`yohiradev`/`t.kidodev` のVLAN20到達）は**この新方針で読み替え**（外部VPNはVLAN10のみ）。全ユーザーの払出は VLAN10 到達（RDP/SSH）に統一する。
