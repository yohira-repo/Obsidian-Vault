...
2. **収集**（リポジトリごと）
   - ベース: `refs/remotes/origin/*` の全ブランチにある `conversations.md`（＝GitHub の内容。他 PC の作業を含む）
   - 上書き: ローカル作業ツリーの `conversations.md`（未 commit 分。同一キーならこちらを優先）
   - 同一 blob SHA は 1 回だけ解析する。
3. **抽出**
   正規表現 `^## (\d{4}-\d{2}-\d{2})[:\s]\s*(.+)$` で見出しを認識し、対象日に一致するセクションを
   次の `## ` 見出しの直前まで本文ごと切り出す。
4. **重複排除**
   キーは `(リポジトリ名, 日付, 正規化タイトル)`。正規化はタイトル前後の空白と末尾コロンの除去。
   優先順位はローカル作業ツリー > コミット日時の新しいブランチ。
5. **ミラー出力**
   `00_Claude/projects/<リポジトリ名>.md` に、未収録のセクションのみを本文ごと末尾追記する。
   ファイルが無ければ `# <リポジトリ名> 作業ログ` を先頭に付けて新規作成する。
...
- **coopbatch** — [[00_Claude/projects/coopbatch#2026-09-06 エラーデータ復旧の準備ロジック検討|エラーデータ復旧の準備ロジック検討]]
<!-- claude-log:end -->
```

- 1 行サマリは `conversations.md` の見出しタイトルをそのまま使う。
- 並び順はリポジトリ名昇順、同一リポジトリ内は `conversations.md` の出現順。
...
- 見出し 2 形式（`YYYY-MM-DD ` と `YYYY-MM-DD:`）の抽出
